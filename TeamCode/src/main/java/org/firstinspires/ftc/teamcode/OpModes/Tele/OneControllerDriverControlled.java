...
@TeleOp(name="One-Controller Driver-Controlled")
public class OneControllerDriverControlled extends LinearOpMode {
    @Override
...
//            telemetry.addData("Back Left", robot.drive.leftBack.getCurrentPosition());
            ...
            //Movement section
            {
                ...
                if (gamepad2.left_bumper) { // Slow Mode
                    x = (0.65 * gpad1MoveSpeed * gamepad2.left_stick_x);
                    y = (0.65 * gpad1MoveSpeed * gamepad2.left_stick_y);
                    r = (0.65 * gpad1RotationSpeed * gamepad2.right_stick_x);
                }
                else if (gamepad2.right_bumper) {  // Boost
                    x = ((1.75 * gpad1MoveSpeed * gamepad2.left_stick_x));
                    y = ((1.75 * gpad1MoveSpeed * gamepad2.left_stick_y));
                    r = ((1.75 * gpad1RotationSpeed * gamepad2.right_stick_x));
                }
                else { // Normal mode
                    x = (.9 * gpad1MoveSpeed * gamepad2.left_stick_x);
                    y = (.9 * gpad1MoveSpeed * gamepad2.left_stick_y);
                    r = (.9 * gpad1RotationSpeed * gamepad2.right_stick_x);
                }
                ...
                //180
                if (turnAroundDebouncer.isPressed(gamepad2.circle)) {
                    robot.drive.rotateLeftEncoder(180);
                }

                //90
                if (turnAroundDebouncer.isPressed(gamepad2.left_trigger > .5)) {
                    robot.drive.rotateLeftEncoder(90);
                }
                if (turnAroundDebouncer.isPressed(gamepad2.right_trigger > 0.5)) {
